...
if (!isset($_GET['payment_id']) || !isset($_GET['id']) || !isset($_GET['date']) || !isset($_GET['hour'])) {
    die("Paramètres manquants. Paiement non validé.");
}
...
$query = "INSERT INTO history_user (id_user, id_event, event_type, date, heure_debut, heure_fin, prix) 
          VALUES (:id_user, :id_event, 'coaching', :date, :heure_debut, :heure_fin, :prix)";
$stmt = $pdo->prepare($query);
...
$pdf->Cell(0, 7, utf8_decode("Client : " . (isset($_SESSION['nom']) ? $_SESSION['prenom'] . ' ' . $_SESSION['nom'] : 'Client #' . $user_id)), 0, 1);
$pdf->Ln(10);
...
$pdf->SetFont('Arial', '', 10);
$pdf->Cell(90, 10, utf8_decode($coaching['titre']), 1, 0, 'L');
$pdf->Cell(30, 10, date("d/m/Y", strtotime($date)), 1, 0, 'C');
$pdf->Cell(30, 10, $heure_debut . ' - ' . $heure_fin, 1, 0, 'C');
...
// Sauvegarde du PDF
$filename = "facture_coaching_" . $payment_id . ".pdf";
$filepath = "invoices/" . $filename;
$pdf->Output($filepath, "F");

// Mettre à jour l'enregistrement en base de données avec le chemin du PDF
$query = "UPDATE history_user SET path_pdf = :path_pdf WHERE id_user = :id_user AND id_event = :id_event AND date = :date";
$stmt = $pdo->prepare($query);
...
$query = "SELECT nom, prenom, email FROM users WHERE id = :id";
$stmt = $pdo->prepare($query);
    ...
    <nav class="bg-white shadow-sm fixed w-full z-50">
        ...
        <div class="sm:hidden hidden" id="mobile-menu">
            <div class="pt-2 pb-3 space-y-1">
                ...
                <a href="coaching.php" class="border-violet text-violet block pl-3 pr-4 py-2 border-l-4 text-base font-medium">
                    Coaching
                </a>
                ...
                <div class="bg-lightgray p-6 rounded-lg mb-8">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600">Coaching :</span>
                            <span class="font-semibold"><?= htmlspecialchars($coaching['titre']); ?></span>
                        </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <span class="font-semibold"><?= $heure_debut; ?> - <?= $heure_fin; ?></span>
                        </div>
